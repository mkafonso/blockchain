...
from apps.backend.elleptic_curve.EllepticCurve import Sha256Point
from apps.backend.helpers.utils import hash160, hash256
from apps.backend.database.database import Database

class Account:
    """
    Pipeline de geração de endereço Bitcoin:

    Private Key (número aleatório seguro)
    → ECC (privateKey * G) → Public Key (x, y)
    → Compressão da chave pública
    → Hash160 (SHA256 + RIPEMD160)
    → Prefixo de rede (0x00 = mainnet)
    → Checksum (duplo SHA256, 4 bytes)
    → Base58 encoding
    = Bitcoin Address final

    Resumo: número aleatório → ponto → hash → endereço legível
    """
    ...
    def create_keys(self):
        ...
        """Checksum"""
        checksum = bytes.fromhex(hash256(newAddr))[:4]
...
if __name__ == '__main__':
    account = Account()
    account.create_keys()
    Database().write(account.__dict__)
